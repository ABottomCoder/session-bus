...
// and with no rate limit it would block at the end of every single turn. State lives in the
...
//
// v0.10.0 rate-limited this with a once-per-session boolean (`armPrompted`). That converged but it
// never RECOVERED: after the single prompt the hook went quiet for the rest of the session's life,
// whether or not the watcher was ever armed. The wake -> drain -> re-arm loop breaks routinely —
// an ESC interrupt tears the background task down (SIGTERM), a deadline is reached and the model
// reports "nothing arrived" without re-arming, a model decides its task is finished and stops. Each
// of those left the session permanently deaf with nothing to say so.
//
// Measured 2026-08-27 on a 10-session bus: `armPrompted=true` on all 9 business sessions, 8 of them
// NOT armed, 6 of those holding unread mail. The latch was the mechanism keeping them deaf.
//
// So the latch becomes a BACKOFF, and the backoff RESETS whenever the session is observed armed.
// Normal operation therefore prompts once per deaf *episode* rather than once per session, and a
// session can never go permanently deaf while a peer is on the bus. Convergence is preserved by the
// widening interval: a session that genuinely cannot arm one (Bash denied, human declined) is
// nagged at 1min, 3min, 10min, then every 30min, not every turn.
// `register()` deliberately does NOT carry this state across a server start. It runs exactly once
// per server (mcp/server.mjs:14; the CLI uses ifAbsent), so it is not a nag vector, and a restarted
// or --resume'd session SHOULD get a prompt promptly rather than inherit a 30-minute wait.
const ARM_BACKOFF_MS = [0, 60_000, 180_000, 600_000, 1_800_000]

// Atomic because listSessions() drops any session whose file will not parse — a torn write here
// would make this session vanish from the bus.
function updateSession(sid, fn) {
  const p = sessionPath(sid)
  const v = readJson(p, null)
  if (!v) return false
  try {
    const tmp = `${p}.tmp${process.pid}`
    writeFileSync(tmp, JSON.stringify(fn(v), null, 2), { mode: 0o600 })
    renameSync(tmp, p)
    return true
  } catch { return false }
}

// Is this session due another arm prompt? Never prompted -> yes. Otherwise only once the backoff
// for the number of prompts already sent has elapsed. An unparseable timestamp reads as due, so a
// corrupt file degrades to "prompt again" rather than to silent deafness.
export function armPromptDue(sid) {
  const v = readJson(sessionPath(sid), null)
  if (!v) return false
  const st = v.armPrompt
  // Migration: a session file written by v0.10.0 carries the old boolean. Treat it as one prompt
  // already sent, with no timestamp, so it becomes due immediately — which is the whole point of
  // this change for the sessions that are deaf right now.
  if (!st) return true
  const at = Date.parse(st.at)
  if (!Number.isFinite(at)) return true
  const n = Number(st.n)
  const wait = ARM_BACKOFF_MS[Math.min(Number.isFinite(n) ? n : 1, ARM_BACKOFF_MS.length - 1)]
  return Date.now() - at >= wait
}

export function markArmPrompted(sid) {
  return updateSession(sid, (v) => ({
    ...v,
    armPrompt: { at: new Date().toISOString(), n: (Number(v.armPrompt?.n) || 0) + 1 },
    armPrompted: undefined,   // drop the v0.10.0 boolean once this session is on the new scheme
  }))
}

// Called on every Stop where the session IS armed, so the NEXT deaf episode gets an immediate
// prompt instead of inheriting a long wait from the previous one. No-ops when there is nothing to
// clear, to avoid a pointless atomic rewrite of the session file on every single turn.
export function clearArmPrompt(sid) {
  const v = readJson(sessionPath(sid), null)
  if (!v || (v.armPrompt === undefined && v.armPrompted === undefined)) return false
  return updateSession(sid, ({ armPrompt, armPrompted, ...rest }) => rest)
}
