...
function itermTitleForTty(tty) {
  if (!tty || process.platform !== 'darwin') return null
...
export function identity() {
  ...
  const sid =
    ...
    `nosid-${pid}`
  const label =
    cleanLabel(process.env.SESSION_BUS_LABEL) ||
    cleanLabel(itermTitleForTty(tty)) ||
    cleanLabel(tmuxTitle()) ||
    basename(process.cwd())
  return { sid, label, pid, tty, cwd: process.cwd() }
}
...
export function listSessions() {
  ...
  for (const f of files) {
    ...
    out.push({
      ...v,
      label: cleanLabel(v.label) || shortId(v.sid),
